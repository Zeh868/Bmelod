/* SPDX-License-Identifier: GPL-3.0-or-later */
/**
 * @file bm_sim_singleton_qemu_rv64_smp.c
 * @brief QEMU RISC-V64 virt SMP 单例驱动（CLINT 定时器 / ns16550a UART / 看门狗桩）
 *
 * 临界区与内存屏障由 `bm_port_arch_riscv64` 提供；M-mode trap 由 boot 层 `_trap_entry` 分发。
 * @maturity E1
 * @author zeh (china_qzh@163.com)
 * @version 1.2
 * @date 2026-07-28
 *
 * @par 修改日志:
 *
 *    Date         Version        Author          Description
 * 2026-06-15       1.0            zeh            正式发布
 * 2026-07-11       1.1            zeh            tick 回调派发接入 arch 层 FPU 守卫（bm_arch_isr_fpu.h，riscv64 IMAC 无 FPU 恒 no-op）
 * 2026-07-28       1.2            zeh            ns16550a 发送轮询加入命名上限，超时返回 BM_ERR_TIMEOUT
 *
 */
#include "bm_drv_timer.h"
#include "bm_drv_uart.h"
#include "bm_hal_uart.h"
#include "bm_drv_wdg.h"
#include "bm_log.h"
#include "bm_types.h"
#include "hal/bm_hal_cpu.h"
#include "riscv64/bm_arch_isr_fpu.h"

#include <stddef.h>
#include <stdint.h>

#define TAG_TIMER "hal_timer"
#define TAG_UART  "hal_uart"
#define TAG_WDG   "hal_wdg"

/** QEMU virt CLINT 基址 */
#define CLINT_BASE         0x02000000UL
/** CLINT 软件中断寄存器 */
#define CLINT_MSIP(h)      (*(volatile uint32_t *)(CLINT_BASE + 4u * (h)))
/** CLINT 机器时间寄存器 */
#define CLINT_MTIME        (*(volatile uint64_t *)(CLINT_BASE + 0xBFF8u))

/**
 * @brief 设置当前 hart 的 CLINT mtimecmp 比较值。
 * @param hart 目标 RISC-V hart 索引。
 * @param val 待写入的 64 位 mtimecmp 比较值，单位为 CLINT tick。
 */
static void clint_set_mtimecmp(uint32_t hart, uint64_t val) {
    volatile uint32_t *lo = (volatile uint32_t *)(CLINT_BASE + 0x4000u + 8u * hart);
    volatile uint32_t *hi = lo + 1;

    *hi = 0xFFFFFFFFu;
    *lo = (uint32_t)val;
    *hi = (uint32_t)(val >> 32);
}

/**
 * @brief 读取 CLINT mtime
 */
static uint64_t clint_get_mtime(void) {
    return CLINT_MTIME;
}

/** QEMU virt 默认 mtime 频率（10 MHz） */
#define RV64_SMP_MTIME_HZ  10000000u

/** ns16550a UART 基址 */
#define UART_BASE          0x10000000UL
#define UART_THR           (*(volatile uint8_t *)(UART_BASE + 0u))
#define UART_LSR           (*(volatile uint8_t *)(UART_BASE + 5u))
/** LSR THRE：发送保持寄存器空 */
#define UART_LSR_THRE      (1u << 5)
/** @brief ns16550a THRE 轮询上限，超出时返回 BM_ERR_TIMEOUT。 */
#define BM_QEMU_RV64_UART_TX_POLL_LIMIT  100000u

static uint32_t g_tick_freq[BM_CONFIG_CPU_COUNT];
static volatile uint32_t g_ticks[BM_CONFIG_CPU_COUNT];
static void (*g_tick_cb[BM_CONFIG_CPU_COUNT])(void);
static uint64_t g_timer_interval[BM_CONFIG_CPU_COUNT];
static int g_timer_armed[BM_CONFIG_CPU_COUNT];

/**
 * @brief 返回当前 hart 索引（越界时回落 0）
 */
static uint32_t rv64_smp_cpu_index(void) {
    uint32_t cpu = bm_hal_cpu_id();

    return (cpu < BM_CONFIG_CPU_COUNT) ? cpu : 0u;
}

/**
 * @brief 为指定 hart 装载下一次 mtimecmp 并开 MIE
 */
static void rv64_smp_timer_arm(uint32_t cpu) {
    uint64_t now = clint_get_mtime();

    clint_set_mtimecmp(cpu, now + g_timer_interval[cpu]);
    g_timer_armed[cpu] = 1;
    /* MTIE(bit7) + MIE：M-mode 机器定时器中断 */
    {
        uintptr_t mtie = 0x80u;
        __asm volatile ("csrs mie, %0" :: "r"(mtie) : "memory");
    }
    __asm volatile ("csrsi mstatus, 8" ::: "memory");
}

/**
 * @brief 初始化当前逻辑 CPU 的定时器。
 * @param freq_hz 定时器频率，单位为 Hz；传入 0 时使用该端口默认频率。
 * @return 成功返回 BM_OK。
 */
static int rv64_smp_timer_init(uint32_t freq_hz) {
    uint32_t cpu = rv64_smp_cpu_index();
    uint32_t hz = (freq_hz > 0u) ? freq_hz : 1000u;

    g_tick_freq[cpu] = hz;
    g_ticks[cpu] = 0u;
    g_timer_interval[cpu] =
        (uint64_t)RV64_SMP_MTIME_HZ / (uint64_t)hz;
    if (g_timer_interval[cpu] == 0u) {
        g_timer_interval[cpu] = 1u;
    }
    rv64_smp_timer_arm(cpu);
    BM_LOGI(TAG_TIMER, "init: cpu=%u freq_hz=%u", (unsigned)cpu, hz);
    return BM_OK;
}

/**
 * @brief 停止定时器设备。
 */
static void rv64_smp_timer_stop(void) {
    uint32_t cpu = rv64_smp_cpu_index();

    g_tick_cb[cpu] = NULL;
    g_timer_armed[cpu] = 0;
}

/**
 * @brief 读取当前逻辑 CPU 的定时器 tick 计数。
 * @return 当前逻辑 CPU 的定时器 tick 计数。
 */
static uint32_t rv64_smp_timer_get_ticks(void) {
    return g_ticks[rv64_smp_cpu_index()];
}

/**
 * @brief 读取当前定时器频率。
 * @return 定时器频率，单位为 Hz；设备无效时返回 0。
 */
static uint32_t rv64_smp_timer_get_freq(void) {
    return g_tick_freq[rv64_smp_cpu_index()];
}

/**
 * @brief 设置定时器回调。
 * @param cb tick 回调；传入 NULL 时解除绑定。
 */
static void rv64_smp_timer_set_callback(void (*cb)(void)) {
    g_tick_cb[rv64_smp_cpu_index()] = cb;
}

const struct bm_timer_driver_api bm_drv_timer_api = {
    rv64_smp_timer_init,
    rv64_smp_timer_stop,
    rv64_smp_timer_get_ticks,
    rv64_smp_timer_get_freq,
    rv64_smp_timer_set_callback,
};

/** @brief tick ISR 内 FPU 现场保存区（riscv64 IMAC 无 FPU，恒 no-op，接线预留）。 */
static uint8_t g_tick_cp0_sa[BM_ARCH_ISR_FPU_SA_SIZE] __attribute__((aligned(16)));

/**
 * @brief M-mode 机器定时器中断（由 startup trap 向量调用）
 *
 * g_tick_cb 派发经 bm_arch_isr_fpu_enter/exit
 * （portable/arch/riscv64/bm_arch_isr_fpu.h）包裹；riscv64 为 IMAC（无 F/D
 * 扩展）恒 no-op，此处接线只为统一调用点，行为不变。
 */
void qemu_rv64_smp_on_timer_irq(void) {
    uint32_t cpu = rv64_smp_cpu_index();
    void (*cb)(void);
    unsigned cp_prev;

    if (!g_timer_armed[cpu]) {
        return;
    }
    clint_set_mtimecmp(cpu, clint_get_mtime() + g_timer_interval[cpu]);
    g_ticks[cpu]++;
    cp_prev = bm_arch_isr_fpu_enter(g_tick_cp0_sa);
    cb = g_tick_cb[cpu];
    if (cb) {
        cb();
    }
    bm_arch_isr_fpu_exit(g_tick_cp0_sa, cp_prev);
}

/**
 * @brief M-mode 软件中断（IPI 唤醒从核后清 MSIP）
 */
void qemu_rv64_smp_on_software_irq(void) {
    uint32_t cpu = rv64_smp_cpu_index();

    CLINT_MSIP(cpu) = 0u;
}

/**
 * @brief 初始化UART端口。
 * @param dev UART 设备实例；当前实现不使用该参数。
 * @param config 设备初始化配置；当前实现不使用该参数。
 * @return 成功返回 BM_OK。
 */
static int rv64_smp_uart_init(const struct bm_hal_uart *dev, void *config) {
    (void)dev;
    (void)config;
    BM_LOGI(TAG_UART, "init: ns16550a");
    return BM_OK;
}

/**
 * @brief 通过UART发送数据。
 * @param dev UART 设备实例；当前实现不使用该参数。
 * @param data 待发送数据缓冲区。
 * @param len 缓冲区中的有效数据长度，单位为字节。
 * @return 成功返回 BM_OK；等待超时时返回 BM_ERR_TIMEOUT。
 */
static int rv64_smp_uart_send(const struct bm_hal_uart *dev,
                            const uint8_t *data, size_t len) {
    (void)dev;
    size_t i;
    uint32_t attempt;

    if (!data || len == 0u) {
        return BM_OK;
    }
    for (i = 0u; i < len; i++) {
        for (attempt = 0u; attempt < BM_QEMU_RV64_UART_TX_POLL_LIMIT;
             ++attempt) {
            if ((UART_LSR & UART_LSR_THRE) != 0u) {
                break;
            }
        }
        if (attempt == BM_QEMU_RV64_UART_TX_POLL_LIMIT) {
            return BM_ERR_TIMEOUT;
        }
        UART_THR = data[i];
    }
    return BM_OK;
}

/**
 * @brief 从UART接收数据。
 * @param dev UART 设备实例；当前实现不使用该参数。
 * @param data 接收数据缓冲区；当前仿真桩不读写该缓冲区。
 * @param max_len 接收缓冲区容量，单位为字节。
 * @return 实际写入接收缓冲区的字节数；无数据或参数无效时返回 0。
 */
static size_t rv64_smp_uart_recv(const struct bm_hal_uart *dev,
                               uint8_t *data, size_t max_len) {
    (void)dev;
    (void)data;
    (void)max_len;
    return 0u;
}

/**
 * @brief 设置UART接收回调。
 * @param dev UART 设备实例；当前实现不使用该参数。
 * @param cb 接收回调；当前仿真桩忽略该参数且不会触发回调。
 */
static void rv64_smp_uart_set_rx_callback(const struct bm_hal_uart *dev,
                                        void (*cb)(uint8_t c)) {
    (void)dev;
    (void)cb;
}

static const struct bm_uart_driver_api g_uart_api = {
    rv64_smp_uart_init,
    rv64_smp_uart_send,
    rv64_smp_uart_recv,
    rv64_smp_uart_set_rx_callback,
};

/** @brief 默认控制台 UART 设备（统一实例模型，见 bm_hal_uart.h）。 */
const bm_hal_uart_t bm_uart_default = { &g_uart_api, NULL };

/**
 * @brief 初始化看门狗仿真桩。
 * @param timeout_ms 看门狗超时时间，单位为毫秒；当前仿真桩不使用该值。
 * @return 成功返回 BM_OK。
 */
static int rv64_smp_wdg_init(uint32_t timeout_ms) {
    (void)timeout_ms;
    BM_LOGI(TAG_WDG, "init: stub");
    return BM_OK;
}

/**
 * @brief 喂养看门狗仿真桩。
 */
static void rv64_smp_wdg_feed(void) {
}

const struct bm_wdg_driver_api bm_drv_wdg_api = {
    rv64_smp_wdg_init,
    rv64_smp_wdg_feed,
};
