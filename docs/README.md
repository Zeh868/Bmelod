...
本文档集以**框架使用**为主：如何跑起来、如何写应用、如何构建、如何移植、如何排障、API 速查；另含少量**设计稿与演进规划**（06 固件升级、07 演进路线），标注「设计稿/规划稿」状态，供审查后转入实现。
...
├── 05-API参考/                 按头文件专题
├── 06-固件升级/                A/B swap 升级方案（设计稿）
└── 07-演进路线/                能力演进路线规划（规划稿）
```
...
| [04-确定性审查报告](04-测试与排障/04-确定性审查报告.md) | WCET/D1-D6 确定性专项审查，含 F-1~F-6 修复记录 |
...
| 文档 | 说明 |
|------|------|
| [01-A-B-swap升级方案设计](06-固件升级/01-A-B-swap升级方案设计.md) | 独立 Bootloader + A/B swap 升级能力设计（设计稿，未实现） |

### 07 演进路线

| 文档 | 说明 |
|------|------|
| [01-框架能力演进路线规划](07-演进路线/01-框架能力演进路线规划.md) | 安全底座 + 通信隔离 + 工程基建 + AI 推理引擎的优先级与依赖路线图（规划稿） |
