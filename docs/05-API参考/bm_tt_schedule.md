...
| 术语 | 含义 |
|------|------|
| **任务表 / activity** | `BM_LET_DEFINE` 声明的一行：周期(`every`×`minor_us`)、相位(`at`)、`domain`、`wcet_us`、纯函数 `step`、输入/输出绑定表 |
...
| **domain** | 任务轴之一：`BM_TT_DOMAIN_ISR`（默认，ISR 内同步跑完）/ `BM_TT_DOMAIN_MAINLOOP`（ISR 只冻结挂起，主循环执行） |
...
BM_LET_DEFINE(task_speed, 1u, 0u, 80u, speed_step, NULL,
...
- `BM_LET_DEFINE` 一行分配 snapshot（输入快照区）、输出双缓冲、per-input
  运行态（`miss`/`stale`/`age_us`/`baseline_seq`）——全部编译期静态数组，
  开发者不用管 bookkeeping。
- `domain` 缺省 `BM_TT_DOMAIN_ISR`；要放到 MAINLOOP 域，`BM_LET_DEFINE`
  后手动 `task_speed.domain = BM_TT_DOMAIN_MAINLOOP;`（宏当前不提供
  MAINLOOP 变体，直接改字段即可，`init` 之前改完成即可）。
...
2. **fail-safe 优先于新鲜度**：输入 `stale=1`（从未发布，或 age 超过
   ...
   `safe_default` 是**非 NULL 强制项**（`BM_LET_DEFINE`/`init` 校验），
   不允许留空。
