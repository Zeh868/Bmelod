...
> 注：`BM_MEMPOOL_DEFINE` 已迁入内部头 `bm/core/bm_mempool_impl.h`，使用处需 `#include "bm/core/bm_mempool_impl.h"`。
...
if (obj != NULL) {
    int publish_rc = bm_event_publish_copy(EVENT_TEMP, 1, obj, sizeof(*obj));
    int free_rc = BM_ERR_BUSY;

    for (uint32_t attempt = 0u;
         attempt < SENSOR_POOL_FREE_RETRY_LIMIT;
         ++attempt) {
        free_rc = bm_mempool_try_free(&sensor_pool, obj);
        if (free_rc != BM_ERR_BUSY) {
            break;
        }
    }
    if (free_rc != BM_OK) {
        /* 必须显式记录/上报：BM_ERR_BUSY 表示有限重试耗尽，对象尚未归还。 */
    }
    /* publish_rc 与 free_rc 分别处理。 */
}
```

`bm_mempool_free()` 仅为兼容包装：它只尝试一次且不返回状态，不能据此认定对象已归还。
共享池需使用 `bm_mempool_try_free()`，在调用方预算内进行命名且有限的重试；重试耗尽时必须保留
`BM_ERR_BUSY` 诊断并安排上层故障处理或补偿。`publish_rc` 与 `free_rc` 应分别处理：事件 payload 已拷贝
不代表对象释放必然成功。

对同一对象再次调用 `bm_mempool_try_free()` 返回 `BM_ERR_INVALID`（fail-stop）；
兼容的 `bm_mempool_free()` 为 `void` 包装，不返回状态，不能用于判定双释放错误。
...
> 注：`BM_BUS_DEFINE` 已迁入内部头 `bm/core/bm_bus_impl.h`，使用处需 `#include "bm/core/bm_bus_impl.h"`。
